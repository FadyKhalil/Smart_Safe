...
//Requests handling
static uint16_t newReq, doneReq;
...
extern void LCD_init()
{
    ...
    for (uint8_t pinIndex = 0; pinIndex < LCD_NUMBER_OF_PINS; pinIndex++)
    {
        configurator.port = Lcd_Pins_cfg[pinIndex].port;
        configurator.pin = Lcd_Pins_cfg[pinIndex].pin;
...
{
    GPIO_writePin(Lcd_Pins_cfg[Lcd_EN_PIN].port, Lcd_Pins_cfg[Lcd_EN_PIN].pin, 0b0);
    GPIO_writePin(Lcd_Pins_cfg[Lcd_RS_PIN].port, Lcd_Pins_cfg[Lcd_RS_PIN].pin, 0b1&((data.type)>>1));
    GPIO_writePin(Lcd_Pins_cfg[Lcd_RW_PIN].port, Lcd_Pins_cfg[Lcd_RW_PIN].pin, 0b1&((data.type)>>0));
    GPIO_writePin(Lcd_Pins_cfg[Lcd_D7_PIN].port, Lcd_Pins_cfg[Lcd_D7_PIN].pin, 0b1&((data.value)>>7));
    GPIO_writePin(Lcd_Pins_cfg[Lcd_D6_PIN].port, Lcd_Pins_cfg[Lcd_D6_PIN].pin, 0b1&((data.value)>>6));
    GPIO_writePin(Lcd_Pins_cfg[Lcd_D5_PIN].port, Lcd_Pins_cfg[Lcd_D5_PIN].pin, 0b1&((data.value)>>5));
    GPIO_writePin(Lcd_Pins_cfg[Lcd_D4_PIN].port, Lcd_Pins_cfg[Lcd_D4_PIN].pin, 0b1&((data.value)>>4));
    GPIO_writePin(Lcd_Pins_cfg[Lcd_D3_PIN].port, Lcd_Pins_cfg[Lcd_D3_PIN].pin, 0b1&((data.value)>>3));
    GPIO_writePin(Lcd_Pins_cfg[Lcd_D2_PIN].port, Lcd_Pins_cfg[Lcd_D2_PIN].pin, 0b1&((data.value)>>2));
    GPIO_writePin(Lcd_Pins_cfg[Lcd_D1_PIN].port, Lcd_Pins_cfg[Lcd_D1_PIN].pin, 0b1&((data.value)>>1));
    GPIO_writePin(Lcd_Pins_cfg[Lcd_D0_PIN].port, Lcd_Pins_cfg[Lcd_D0_PIN].pin, 0b1&((data.value)>>0));
    GPIO_writePin(Lcd_Pins_cfg[Lcd_EN_PIN].port, Lcd_Pins_cfg[Lcd_EN_PIN].pin, 0b1);
}
...
extern void LCD_Task()
{
    ...
    static uint8_t Lcd_stateTickCount[] = {
...
res_t LCD_displayCharacter(uint8_t data)
{
    Lcd_Request_t req = {
...
// example:
//          LCD_Goto(4,1);
extern res_t LCD_Goto(uint8_t x, uint8_t y)
{
    Lcd_Request_t req = {
...
//      - this function needs to be used inside a task scheduled to os
//        schedule in order to work correctly
// Args:    Num          :(int32_t) the displayed number
// Return:  returnStatus    :(res_t) the status of the call 
//                          (Status_Ok: register,
//                          Status_busy: not registered)
// example:
//          Lcd_displayNumber(12);
extern res_t Lcd_displayNumber(int32_t Num)
{
	res_t Loc_enuStatus = Status_Ok;
	int32_t Loc_s32Val;
    uint8_t Loc_u8Itr;
    Lcd_Request_t req;
	static uint8_t Loc_au8AllDigits[11]; // from -2 Billion to 2 Billion (1 bit sign and 10 bits number)
